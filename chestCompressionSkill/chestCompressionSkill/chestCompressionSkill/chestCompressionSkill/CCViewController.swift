...
//    let image = UIImage(named: "art.scnassets/test1.jpeg")
...
//    let radTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(updateRad), userInfo: nil, repeats: true) // timer to update majRadius
    ...
    @IBOutlet weak var gaugeImage: UIImageView!
    @IBOutlet weak var guageNeedle: UILabel!
    var timer = Timer()
    @objc func timerAction() {
        if speedTouchCt < 5 {
            //too slow
            if currentColor == "green" {
                guageNeedle.transform = CGAffineTransform(rotationAngle: -45)
            } else if currentColor == "red" {
                guageNeedle.transform = CGAffineTransform(rotationAngle: -90)
            }
...
//            speedButton.tintColor = UIColor.green
    ...
    @IBAction func startButton(_ sender: UIButton) {
        ...
        let timer2 = Timer.scheduledTimer(withTimeInterval: 33.2, repeats: false) { timer in
    ...
    override func viewDidLoad() {
        ...
        pressureBar.transform = CGAffineTransform(rotationAngle: .pi * 3 / 2).translatedBy(x: 0, y: -50)
...
//        sceneView.scene = scene
        ...
        //gauge stuff
//        gaugeImage.isHidden = false
//        imageView.image = UIImage(named:"foo")
        gaugeImage.image = UIImage(named: "test1.jpeg")
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if bodyPlace == true {
            if !bodyArray.isEmpty {
                bodyArray.removeFirst(1)
//                for bodies in bodyArray {
//                    bodies.removeFromParentNode()
//                }
            }
            if let touch = touches.first {
                ...
                if let hitResult = results.first {
                    ...
                    if let bodyNode = bodyScene.rootNode.childNode(withName: "please", recursively: true) { // please, Object-1
                        ...
                        bodyNode.rotation = SCNVector4(0, 270, 0, 0) //x,y,z,w
                        bodyArray.append(bodyNode)
                        sceneView.scene.rootNode.addChildNode(bodyArray.first!)
                    }
                }
            }
            bodyPlace = false
...
//        }
        ...
        if rad > 60 { // 60 is just a temp number
            ...
            pressureBar.tintColor = UIColor.gray
        }
...
//                }
            ...
            if let touch = touches.first {
                ...
                let results = sceneView.hitTest(touchLocation, types: .existingPlaneUsingExtent)
                if let hitResult = results.first {
                    let bodyScene = SCNScene(named: "art.scnassets/up.scn")! //down.scn, AnyConv.com__manbody copy.scn, success copy.scn, up.scn
                    if let bodyNode = bodyScene.rootNode.childNode(withName: "up", recursively: true) { // please, Object-1, untitled_2, up
                        bodyNode.position = SCNVector3(
                            x: hitResult.worldTransform.columns.3.x,
                            y: hitResult.worldTransform.columns.3.y - (bodyNode.boundingSphere.radius / 2),
                            z: hitResult.worldTransform.columns.3.z
